...
class event_listing extends acf_Field
{
	...
	*-------------------------------------------------------------------------------------*/
	...
	function __construct($parent)
	{
    	...
    	// set name / title
    	$this->name = 'event_listing'; // variable name (no spaces / special characters / etc)
		$this->title = __("Event Listing",'acf'); // field label (Displayed in edit screens)
	...
	*-------------------------------------------------------------------------------------*/
	...
	function create_field($field)
	{
		...
		// Select database
		mysql_select_db ("cie", $con) or die ("Unable to select database");
		...
		$res = mysql_query("SELECT * FROM  wp_ai1ec_events WHERE DATE(start) >= DATE(NOW()) ORDER BY start ASC", $con)
		    or die ("Unable to run query");
		...
		while ($row = mysql_fetch_array($res)) {
			...
			$sel = ($row["post_id"] == $val) ? 'selected = "selected"' : '';

			echo '<option value="'.$row["post_id"].'"'.$sel.'>'.$row2["post_title"].'</option>';
