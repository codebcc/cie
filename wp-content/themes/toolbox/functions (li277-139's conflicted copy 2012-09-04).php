...
function event_list($amount = 0) {
	$amount = ($amount==0) ? "" : " LIMIT ".$amount;
    echo '<ul>';
    global $wpdb;
    $myrows = $wpdb->get_results( "SELECT * FROM  wp_ai1ec_events WHERE DATE(start) >= DATE(NOW()) ORDER BY DATE(start)".$amount);
    $count=0;
...
//add meta box
add_meta_box('categorydiv', __('Categories'), 'post_categories_meta_box', 'performers', 'side', 'low');
}
